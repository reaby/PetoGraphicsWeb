...
describe('/api/files', () => {
    ...
    describe('GET /api/files', () => {
        it('should return 200', (done) => {
            request(app).get('/api/files').expect(200).end(done);
        });

        it('should have Content-Type application/json', (done) => {
            request(app).get('/api/files').expect('Content-Type', /json/).end(done);
        });

        it('should return list of files', (done) => {
            request(app)
                .get('/api/files')
                .expect(200, ['config.json', 'test.txt', 'test1.txt'])
                .end(done);
        });
    });
    ...
    describe('DELETE /api/files/:file', () => {
        it('should return 200', (done) => {
            request(app).delete('/api/files/test.txt').expect(200).end(done);
        });
        ...
        it('should return 404 when file is not found', (done) => {
            request(app).delete('/api/files/test2.txt').expect(404).end(done);
        });
    });
